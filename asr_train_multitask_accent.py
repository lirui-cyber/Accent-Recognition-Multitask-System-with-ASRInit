...
from espnet.asr.pytorch_backend.asr_init import load_trained_modules_include_ctc
...
from espnet.utils.io_utils_ZJC2 import LoadInputsAndTargets
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        assert len(batch) == 1
        # logging.warning("length batch[0]: {}".format(batch[0][0]))
        # logging.warning("length batch[1]: {}".format(batch[0][1]))

        xs, ys, utt_name = batch[0]
        # utt_name = batch[0][1]
        # perform subsampling
        ...
        return xs_pad, ilens, ys_pad, utt_name
...
def train(args):
    ...
    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        ...
        model = load_trained_modules_include_ctc(idim_list[0], odim, args)
    else:
        ...
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    if args.recog_mode == "asr":
                        nbest_hyps = model.recognize(
                            feat, args, train_args.char_list, rnnlm
                        )
                    else:
                        nbest_hyps = model.recognize_accent(
                            feat, args, train_args.char_list, rnnlm
                        )
