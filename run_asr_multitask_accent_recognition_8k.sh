...
if [ ! -z $step01 ]; then
   echo "extracting filter-bank features and cmvn for 8k data"
   for i in $train_set $valid_set $recog_set;do
       utils/fix_data_dir.sh $data/$i
       steps/make_fbank_pitch.sh --cmd "$cmd" --nj $nj --write_utt2num_frames true --pitch_config conf/pitch-8k.conf --fbank_config conf/fbank-8k.conf \
...
if [ ! -z $step05 ]; then
    ...
    if  [ ${epoch_stage} -gt 0 ]; then
        echo "stage 5: Resume network from epoch ${epoch_stage}"
